              ...
              child: Column(
                children: [
                  Padding(
                    ...
                    child: Container(
                      child:  Text("Ticket Booking",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 25,color: Color(0xff4c505b)),),
                    ),
                  ),
                  SizedBox(
                    height: 10.0,
  ...
  Submit() async{
    ...
    if(_formkey.currentState!.validate()){
      try{
        ...
        if(widget.id ==null){
          await FirebaseFirestore.instance.collection("ticketbook").add({
            'name': username.text,
            'age': age.text,
            ...
            Get.snackbar("Ticket Book", "Ticket book successfully",snackPosition: SnackPosition.BOTTOM);
            Navigator.of(context).pop();
          });
        }else{//update code
          await FirebaseFirestore.instance.collection("ticketbook").doc(widget.id).update({
            'name': username.text,
            'age': age.text,
            'mobileno':number.text,
            'gender':selectedGender,
            ...
            Get.snackbar('Update Ticket Book', 'Update Ticket book succesfully',snackPosition: SnackPosition.BOTTOM);
            Navigator.of(context).pop();
          });
        }
        setState(() {
